...
def main(df_train, df_test, model_name, lang, k, num_epochs=20, save_model_flag=True):
    ...
    metrics_path = f"outputs/metrics_{lang}_{k}.txt"
    ...
    df_test.to_csv(f"outputs/with_predictions_{lang}_{k}.txt", index=False)
    print(f"Test dataframe with predictions saved to outputs/with_predictions_{lang}_{k}.txt")

if __name__ == "__main__":
    # Placeholder paths for your dataset files
    df_train = pd.read_csv("test_data/en_train.csv") 
    df_test = pd.read_csv("test_data/en_dev.csv")
    lang = "en"
    k = "0"
